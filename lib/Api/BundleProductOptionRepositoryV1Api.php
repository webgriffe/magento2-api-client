...
class BundleProductOptionRepositoryV1Api
{
     ...
     * @param int $optionId  (required)
     * @throws \Webgriffe\MagentoApiClient\ApiException on non-2xx response
     * @return array of \Webgriffe\MagentoApiClient\Model\BundleDataOptionInterface, HTTP status code, HTTP response headers (array of strings)
     */
    public function bundleProductOptionRepositoryV1GetGetWithHttpInfo($sku, $optionId)
    {
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\Webgriffe\MagentoApiClient\Model\BundleDataOptionInterface',
                '/V1/bundle-products/{sku}/options/{optionId}'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\Webgriffe\MagentoApiClient\Model\BundleDataOptionInterface', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Webgriffe\MagentoApiClient\Model\BundleDataOptionInterface', $e->getResponseHeaders());
                    $e->setResponseObject($data);
                    break;
                case 400:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Webgriffe\MagentoApiClient\Model\ErrorResponse', $e->getResponseHeaders());
